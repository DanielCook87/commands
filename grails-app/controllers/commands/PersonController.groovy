...
class PersonController {
    ...
    def save(PartialPerson person) {
        if(!person){
            // response.sendError(404)
            flash.message = "nooooo!"
            redirect action:'index'
            return
        }
        println person
        println "Is valid? ${person.validate()}"
        ...
        Person p = new Person()
        p.properties = person.properties
        p.name = "Bobbert - ${System.nanoTime()}"

        println "valid? ${p.validate()}"

        try {
            personService.save(p)
        } catch (ValidationException e) {
            respond p.errors, view:'create'
            return
        }
        ...
        request.withFormat {
            form multipartForm {
                flash.message = message(code: 'default.created.message', args: [message(code: 'person.label', default: 'Person'), p.id])
                redirect p
            }
